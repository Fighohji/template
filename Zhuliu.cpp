...
int u, v, w;
int g[N][N], inCost[N],
    pre[N]; // inCost[u]存储u的最小入边花费。pre[u]存储u的前驱结点。
int id[N],
    vis[N]; // id[u]表示u重新编号的点，这个数组是为了处理有向环的。vis[u]表示u是通过vis[u]来访问的。
struct node {
    int u, v, w;
} edges[N];
int zhuliu() {
    ...
    while (true) {
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            while (vis[v] != i && id[v] == -1 && v != root) {
                vis[v] = i;
                v = pre[v]; // 不断迭代前驱结点，知道更新到根结点。若是环，当处于vis[v]
                            // = i时即会退出，即回到起点。
            }
            if (v != root && id[v] == -1) {
