...
/*  int dfn, id[maxn]; // st表求lca,能快一点,费空间
    ...
    int cnt, head[maxn], nxt[maxn], e[maxn];
    int stk[maxn], top, tmp[maxn];
    ...
    void build(int k) {
        sort(tmp + 1, tmp + 1 + k, [&](int x, int y) { return id[x] < id[y]; });
        stk[top = 1] = 1; cnt = 0; head[1] = 0; //放入根节点
        for (int i = (tmp[1] == 1 ? 2 : 1), l; i <= k; ++i) {// 根节点不要重复添加
